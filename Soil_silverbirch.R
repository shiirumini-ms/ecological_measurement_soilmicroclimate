...
soil_pendula <- soil_birch[soil_birch$Species == "Silver Birch (Betula pendula)", ]
View(soil_pendula)
...
# No significant relation, p = 0.943. 
# Mean 0.5 m: 17.1818 cm, 0.7 m: 17.39091
...
# No significant relation, p = 0.751. 
...
p1 <- ggbarplot(
  ...
  aes(x = Distance, y = max(soil_pendula$Depth_cm) + 1, label = "n = 11"),
...
p2 <- ggbarplot(
  ...
  aes(x = Distance, y = max(soil_birch$Depth_cm) + 1, label = "n = 11"),
  position = position_nudge(x = -0.25, y = 45),
  inherit.aes = FALSE,
  size = 4
)
...
cor.test(soil_pendula$Depth_cm, soil_pendula$Moisture_vv, method = "spearman")

# Spearman correlation test: 
# p-value = 0.6289, rho = 0.1091013
# Not linearity between soil moisture and depth. 

# Visualisation
p3 <- ggscatter(soil_pendula, 
                x = "Moisture_vv", 
                y = "Depth_cm", 
                combine=FALSE)
...
# No significant difference by tree distance if we only look at Betula. 
# Why? Age group difference? Silver birch are all quite young opposed to other tree species? 
# Rooting depth? 
# Soil microclimate community? 
# Soil moisture, they like moist soil? 
# Particular planting methodology used for betula pendula? 
# I would like to know more about this. 
# This could be more on the future discussion. 
# Email Nature Scot. 
# Perhaps I plot DBH (cm) as X variable and Y variable as a difference between 0.5 and 7.0., how would that look like? 

mean_DBH = mean(soil_pendula$DBH_cm)
hist(soil_pendula$DBH_cm, 
     breaks = 10)
# Predominantly around 5-10 cm. ~15 years. 